...
namespace _10_3_PhongLightingModelInViewSpace {
    void main();
...
void _10_3_PhongLightingModelInViewSpace::main() {
    manager.init();
    ...
    Geometry::CubeV2 obj {};
    Shader obj_shader {
        "./shaders/10-lighting/phong-lighting-model-in-view-space.vs",
        "./shaders/10-lighting/phong-lighting-model-in-view-space.fs"
    };
    obj_shader.use();
    obj_shader.set_vec3("u_color", object_color);
    obj_shader.set_vec3("u_light_color", light_color);
    obj_shader.set_vec3("u_light_source_position", light_source_position);
    ...
    obj_shader.set_float("u_ambient_intensity", .1f);
    obj_shader.set_float("u_specular_intensity", .5f);
    obj_shader.set_int("u_specular_shininess", 128);
...
void _10_3_PhongLightingModelInViewSpace::mouse_callback(GLFWwindow* window, double x, double y) {
    camera->process_cursor(x, y);
}

void _10_3_PhongLightingModelInViewSpace::scroll_callback(GLFWwindow* window, double x_offset, double y_offset) {
    *fov -= y_offset;
