...
namespace _13_4_SpotLightSoftEdge {
    void main();
...
void _13_4_SpotLightSoftEdge::main() {
    manager.init();
    manager.setup_camera_events(mouse_callback, scroll_callback);

    Geometry::CubeV2 obj {};
    Shader obj_shader {
        "./shaders/13-light-casters/spot-light-soft-edge.vs",
        "./shaders/13-light-casters/spot-light-soft-edge.fs"
    };
    obj_shader.use();
    ...
    manager.render([
        ...
        obj_shader.set_float("u_light.outer_cut_off", glm::cos(glm::radians(*spotlight_angle + 5.0f)));
...
void _13_4_SpotLightSoftEdge::mouse_callback(GLFWwindow* window, double x, double y) {
    camera->process_cursor(x, y);
}

void _13_4_SpotLightSoftEdge::scroll_callback(GLFWwindow* window, double x_offset, double y_offset) {
    *fov -= y_offset;
...
vector<float> _13_4_SpotLightSoftEdge::calc_vertex_normals(float vertices[], float vertices_count) {
    vector<float> result {};
