...
void _13_2_PointLight::main() {
    ...
    Geometry::CubeV2 obj {};
    Shader obj_shader {
        "./shaders/13-light-casters/point-light.vs",
        "./shaders/13-light-casters/point-light.fs"
    };
    obj_shader.use();
    ...
    // point light also have attenuation factor
    ...
    manager.render([
        ...
        glm::vec3 light_position {
            sin(context->get_time()) * 5.0f,
            1.0f,
            cos(context->get_time()) * 5.0f - 4.0f
        };

        light_source_shader.use();
        Transform light_source_model {};
        light_source_model.translate(light_position);
        light_source_model.scale(.2f, .2f, .2f);
        light_source_shader.set_coordinates(light_source_model, view, projection);
        light_source.render();
        ...
        obj_shader.set_vec3("u_light.position", light_position);
...
void _13_2_PointLight::mouse_callback(GLFWwindow* window, double x, double y) {
    camera->process_cursor(x, y);
}

void _13_2_PointLight::scroll_callback(GLFWwindow* window, double x_offset, double y_offset) {
    *fov -= y_offset;
...
vector<float> _13_2_PointLight::calc_vertex_normals(float vertices[], float vertices_count) {
    vector<float> result {};
