...
namespace _13_1_DirectionalLight {
    void main();
...
void _13_1_DirectionalLight::main() {
    manager.init();
    ...
    Geometry::CubeV2 obj {};
    Shader obj_shader {
        "./shaders/13-light-casters/directional-light.vs",
        "./shaders/13-light-casters/directional-light.fs"
    };
    obj_shader.use();
    ...
    // this time we set the direction of the directional light
    obj_shader.set_vec3("u_light.direction", -.2f, -1.0f, -.3f);
...
void _13_1_DirectionalLight::mouse_callback(GLFWwindow* window, double x, double y) {
    camera->process_cursor(x, y);
}

void _13_1_DirectionalLight::scroll_callback(GLFWwindow* window, double x_offset, double y_offset) {
    *fov -= y_offset;
...
vector<float> _13_1_DirectionalLight::calc_vertex_normals(float vertices[], float vertices_count) {
    vector<float> result {};
